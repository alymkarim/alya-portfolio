...
export const projects: Project[] = [
  {
    ...
    shortDescription:
      "Full-stack research paper analysis platform with AI-powered insights, search and comparison.",
    description:
      "ResearchIQ lets researchers upload PDF papers and get structured analysis back in seconds. It extracts objectives, methodology, findings, strengths and limitations using either heuristic pattern matching or LLM-powered analysis. Features include semantic search across paper collections, side-by-side comparison, conversational Q&A with papers, paper discovery from Semantic Scholar and arXiv, visualizations, notes and annotations, and export to PDF or DOCX.",
    ...
    architecture: [
      "PDF upload with automatic metadata extraction (title, authors, abstract)",
      "Text chunking with page-aware TF-IDF and FAISS vector search",
      "Heuristic fallback analysis that works without any API key",
      "LLM integration via OpenAI-compatible APIs (OpenAI, Groq, Ollama)",
      "Conversational Q&A over papers using retrieval-augmented generation",
      "External paper discovery via Semantic Scholar and arXiv APIs",
      "React frontend with workspace-based layout and dark/light themes",
      "FastAPI backend with PostgreSQL via Supabase",
    ],
    highlights: [
      "Works offline with built-in heuristic analysis",
      "Multi-model LLM support (OpenAI, Groq, Anthropic, Ollama)",
      ...
      "Quick, standard and deep analysis levels",
      "Paper discovery from external academic databases",
    ],
    challenges: [
      "Getting reliable structured JSON from smaller local LLMs",
      "Handling PDFs with inconsistent formatting and encoding",
      "Preventing LLM hallucinations by grounding responses in source text",
      "Managing cold start times on free-tier hosting",
    ],
    lessons: [
      "Heuristic fallbacks are essential when LLMs are unreliable or unavailable",
      "Retrieval quality matters more than LLM capability for RAG systems",
      "Structured outputs are more useful than free-form chat for research",
      "Free-tier hosting requires keep-alive strategies and graceful degradation",
    ],
  },
  {
    id: "cartos",
